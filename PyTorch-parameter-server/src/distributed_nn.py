...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        master_fc_nn.start_updated()
        current_time = time.time() - current_time
        print("Done sending messages to workers! , total time takken by master " + str(current_time))
    else:
        ...
        worker_fc_nn.train_updated(train_loader=train_loader, test_loader=test_loader)
        worker_time = time.time() - worker_time

        print("Now the next step is: {}".format(worker_fc_nn.next_step))
        print("totla time by worker: {} is {} ".format(worker_fc_nn.rank, worker_time))
