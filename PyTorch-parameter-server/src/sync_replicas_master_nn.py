...
class SyncReplicasMaster_NN(NN_Trainer):
    ...
    def start(self):
        for i in range(1, self._max_steps+1):
            ...
            self._bcast_weight()
            t= time.time() -t
            self.send_time = self.send_time + t

            t= time.time()
            self._recv_grads()
            t = time.time() -t
            self.recieve_time= self.recieve_time + t

            print("total time taken master time now send time = {}  , recieve rtime = {}".format(self.send_time , self.recieve_time))
    ...
    def _bcast_weight(self):
        for layer_idx, layer in enumerate(self.network.parameters()):
            #print(self._grad_aggregate_buffer[layer_idx])
            #layer_weight = self.compressor.compress(self._grad_aggregate_buffer[layer_idx])
            #print("compressed")
            #print(layer_weight)
            #dist.broadcast(layer_weight[0], src=0)
            #dist.broadcast(layer_weight[1], src=0)

            #layer_weight = layer.detach()
            #layer_weight = self.compressor.compress(layer_weight)
            #print("master "+ str(layer_weight[1]))
            #dist.broadcast(layer_weight[0], src=0)
            #dist.broadcast(layer_weight[1], src=0)

            #print(self._grad_aggregate_buffer[layer_idx])
            layer_weight = self._grad_aggregate_buffer[layer_idx]
            #print("compressed")
            #print(layer_weight)
    ...
    def _recv_grads(self):
        for layer_idx, layer in enumerate(self.network.parameters()):
            element = [torch.Tensor(1)] * (self.world_size)
            dummpy_grad = self.grad_accumulator.gradient_aggregator[layer_idx][0]
            #print("master   "+str(self.grad_accumulator.gradient_aggregator[layer_idx][0].shape))
            dist.gather(dummpy_grad, self.grad_accumulator.gradient_aggregator[layer_idx], dst=0)

            #print("master node" + str(self.grad_accumulator.gradient_aggregator[layer_idx][1]))
            #dist.gather(element[0],element, dst=0)
            #print("master node shape" + str(self.grad_accumulator.gradient_aggregator[layer_idx][1]))

            #print(element)
            #self.grad_accumulator.gradient_aggregator[layer_idx] = self.decompress(self.grad_accumulator.gradient_aggregator[layer_idx],element)
            #print(self.grad_accumulator.gradient_aggregator[layer_idx][1])
            self.aggregate_gradient(layer_idx=layer_idx, gradient=self.grad_accumulator.gradient_aggregator[layer_idx])

    def decompress(self,gradient, norm ):
      result = []
      result.append(gradient[0])
      gradient = gradient[1:]
      norm = norm[1:]
      for x,y in zip(gradient,norm):
        result.append(self.compressor.decompress((x,y)))
      return result
